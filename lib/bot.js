...
class Bot {
  ...
  sendToIRC(message) {
    ...
    let discordClient = this.discord;

    let nick = () => {
      let user = discordClient.users.get(message.author.id)
      return message.server.detailsOf(user).nick || (user ? user.username : message.author.name)
    }
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        this.ircClient.say(ircChannel, `+<${username}> ${text}`);
      } else {
        if (text !== '') {
