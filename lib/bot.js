...
class Bot {
  constructor(options) {
    ...
    this.rawLog = options.rawLog === true;
    ...
    logger.add(logger.transports.File, { filename: './logs/log.log' });
  }
  ...
  connect() {
    ...
    logger.info(`${this.server}, ${this.nickname}`);
  ...
  attachListeners() {
    ...
    // untested ribot code - might be needed for ircnet to prevent k-lines
    this.ircClient.on('ctcp-version', (from, to, message) => {
      this.ircClient.ctcp(from, "notice", "discord nab bot v0.1");
    });
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        this.ircClient.say(ircChannel, `+<${displayUsername}> ${text}`);
      } else {
        if (text !== '') {
          // Convert formatting
